component output="false" {
    public array function search(required string query, numeric limit=5) output="false" {
        ...
        var q = queryExecute(
            "SELECT knowledge_document_id, document_type, document_title, document_content, chunk_text FROM sc_knowledge_documents WHERE document_status = 'active' AND (document_title ILIKE ? OR document_content ILIKE ? OR embedding_text ILIKE ?) ORDER BY knowledge_document_id DESC LIMIT ?",
            [
