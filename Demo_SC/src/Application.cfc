component {
    this.name = "SmartCityAgentDemo";
    this.applicationTimeout = createTimeSpan(0, 2, 0, 0);
    this.sessionManagement = true;
    this.sessionTimeout = createTimeSpan(0, 1, 0, 0);
    this.datasource = "demo_sc";
    this.mappings["/services"] = getDirectoryFromPath(getCurrentTemplatePath()) & "services";
    this.mappings["/demo_sc"] = getDirectoryFromPath(getCurrentTemplatePath());
    this.charset.web = "utf-8";
    this.charset.resource = "utf-8";

    function onApplicationStart() {
        application.appName = "SmartCity Agent Demo";
